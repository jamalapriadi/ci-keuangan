...
class Lap_laba_rugi extends CI_Controller {
	...
	public function index()
	{
		...
		if(!empty($cek)){
			...
			$d['content'] = $this->load->view('lap_laba_rugi/view', $d, true);		
	...
	public function view_data()
	{
		...
		if(!empty($cek)){
			...
			$text = "SELECT * FROM rekening WHERE level=0 AND substr(no_rek,1,1)>='4' ORDER BY no_rek";
			$d['data'] = $this->app_model->manualQuery($text);
			
			$this->load->view('lap_laba_rugi/view_data',$d);
	...
	public function cetak_data()
	{
		...
		if(!empty($cek)){
			...
			$this->load->view('lap_laba_rugi/cetak_data',$d);
