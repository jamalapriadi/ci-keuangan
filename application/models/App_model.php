   ...
   * @keterangan : Model untuk menangani semua query database aplikasi
    **/
    ...
    public function buat_kode(){
		$this->db->select('RIGHT(po_intern.no_po,3) as kode', FALSE);
	...
	public function KodeClient(){
		$this->db->select('RIGHT(client.id_client,3) as kode', FALSE);
	...
	public function CariNamaClient($id){
		$text = "SELECT * FROM client a left join po_intern b on a.id_client=b.id_client WHERE a.id_client='$id'";
		$data = $this->app_model->manualQuery($text);
		if($data->num_rows() > 0 ){
			foreach($data->result() as $t){
				$hasil = $t->nama_client;
	...
	public function ClientPenjualan($id){
		$text = "SELECT * FROM client a left join order_penjualan b on a.id_client=b.id_client WHERE a.id_client='$id'";
		$data = $this->app_model->manualQuery($text);
		if($data->num_rows() > 0 ){
			foreach($data->result() as $t){
				$hasil = $t->nama_client;
